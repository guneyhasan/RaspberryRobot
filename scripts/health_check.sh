...
if ! ping -c 1 -W 3 8.8.8.8 &>/dev/null; then
  ONNX=$(find "$ROOT/models/tr_TR-ahmet-medium" -maxdepth 1 -name '*.onnx' 2>/dev/null | head -1)
  JSON=$(find "$ROOT/models/tr_TR-ahmet-medium" -maxdepth 1 -name '*.onnx.json' 2>/dev/null | head -1)
